...
class CategoricalImageDataGenerator():
    def __init__(self, stream, numClasses):
    ...
    def transformToCategorical(self, im):
        fun = np.vectorize(self.mapNums)
        return to_categorical(fun(im), self.numClasses)

    def __iter__(self):
        return self

    def __next__(self):
        nextBatch = next(self.stream)
        result = []
        for im in nextBatch:
            result.append(self.transformToCategorical(im))
